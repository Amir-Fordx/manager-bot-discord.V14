...
module.exports = {
    ...
    .addStringOption(option =>
        option
        .setName('platform')
        .setDescription('meme platform (optinal)')
        .addChoices(
    ...
    async execute(interaction) {
        const {guild, options, memebr} = interaction;
        ...
        async function giphyMeme() {
            await fetch('https://api.giphy.com/v1/gifs/random?api_key=J8QA2qLAUJUzSTuLI8Qhyy357q52FJfd&tag=&rating=g').then(async res => {
                let meme = await res.json()
